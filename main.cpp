...
struct Color {
  enum { COLOR_VECTOR_SIZE = 4 };
	float color[COLOR_VECTOR_SIZE] = { 0.f, 0.f, 0.f, 0.f };
...
struct Point {
	...
	Point& multMatrix( const double* matrix ){
...
struct Shape {
  ...
  void setLineColor(float r, float g, float b, float a = 1.0f) {
	...
	Shape& multMatrix( const double* matrix ){
		for( auto& p : points ){
			p.multMatrix(matrix);
		}
		return *this;
...
struct Scene {
	...
	double projection[PROJECTION_SIZE] = { 1, 0, 0, 0, 1, 0, 0, 0, 1 };
  ...
  Color web_line_color;
	...
	void set_projection( double left, double right, double top, double bottom ){
		memset(projection, 0, sizeof(projection));

		const double w = right - left;
		const double l = top - bottom;
	...
	void set_projection( double projection[PROJECTION_SIZE] ){
		memcpy(this->projection, projection, sizeof(this->projection));
	}
